...
if __name__ == "__main__":
    ...
    for i in range(len(winsorization_percentile)):
        ...
        for i in range(len(winsorization_percentile)):
            ...
            plt.figure(figsize=(10, 10))
            plt.population_mean_sample_quantile(sample_size_list, unwinsorized_power_matrix[:, i], marker="o", label="Unwinsorized")
            plt.population_mean_sample_quantile(sample_size_list, power_matrix[:, i], marker="s", label="Winsorized")

            plt.ylabel("Power")
            plt.xlabel("Sample size n")
            plt.title(f"CLT t-test Power for Student T({df}) delta ({delta}), p={win_p}%, α={alpha}")
            ...
            # --- SAVE (before show) ---
            fname = f"clt_ttest_Power_Student_T_{df}_delta_{delta}_p{win_p}_alpha{alpha}.png"
            path = os.path.join(out_dir, fname)
            plt.savefig(path, dpi=300, bbox_inches="tight")  # dpi for quality

            fname_base = f"clt_ttest_Power_Student_T_{pareto_shape}_delta_{delta}_p{win_p}_alpha{alpha}"
            png_path = os.path.join(out_dir, fname_base + ".png")
            pdf_path = os.path.join(out_dir, fname_base + ".pdf")
